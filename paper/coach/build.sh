...
OUTPUT_PDF="${1:-$BUILD_DIR/coach-foremoz-whitepaper.pdf}"
...
if [[ ! -x "$PDF_ENGINE" ]]; then
  ...
  echo "  PDF_ENGINE=/Library/TeX/texbin/xelatex ./paper/coach/build.sh" >&2
  exit 1
fi
...
build_pdf "$OUTPUT_PDF" "Foremoz Coach Whitepaper" "${files[@]}"
