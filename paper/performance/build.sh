...
OUTPUT_PDF="${1:-$BUILD_DIR/performance-foremoz-whitepaper.pdf}"
...
if [[ ! -x "$PDF_ENGINE" ]]; then
  ...
  echo "  PDF_ENGINE=/Library/TeX/texbin/xelatex ./paper/performance/build.sh" >&2
  exit 1
fi
...
generate_diagram_pngs
build_pdf "$OUTPUT_PDF" "Foremoz Performance Whitepaper" "${files[@]}"
