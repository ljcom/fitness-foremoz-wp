...
OUTPUT_PDF="${1:-$BUILD_DIR/creator-events-foremoz-whitepaper.pdf}"
...
if [[ ! -x "$PDF_ENGINE" ]]; then
  ...
  echo "  PDF_ENGINE=/Library/TeX/texbin/xelatex ./paper/creator-events/build.sh" >&2
  exit 1
fi
...
generate_diagram_pngs
build_pdf "$OUTPUT_PDF" "Foremoz Creator Events Whitepaper" "${files[@]}"
